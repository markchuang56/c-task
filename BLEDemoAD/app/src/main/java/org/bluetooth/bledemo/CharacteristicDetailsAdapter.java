...
public class CharacteristicDetailsAdapter extends BaseAdapter {
	...
	public void setCharacteristic(BluetoothGattCharacteristic ch) {
		...
		Log.e("---++---", "H2BT SET CHARACTERISTIC" + " " + uuid);


		if(uuid.equals(BleDefinedUUIDs.Characteristic.H2BT_CABLE_CHARACTERISTIC)) {
			mBleWrapper.mH2BTCharacteristicCable = ch;
			Log.e("---++---", "H2BT CHARACTERISTIC CABLE");
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.LOCAL_TIME_UUID)) {
			mBleWrapper.mRoCharacteristicCurrentTime = ch;
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.BG_CONTROL_UUID)) {
			mBleWrapper.mRoCharacteristicControl = ch;
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.BG_MEASUREMENT_UUID)) {
			mBleWrapper.mRoCharacteristicMeasurement = ch;
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.BG_CONTEXT_UUID)) {
			mBleWrapper.mRoCharacteristicContext = ch;
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.BG_SN_UUID)) {
			mBleWrapper.mRoCharacteristicSerialNumber = ch;
		}else if(uuid.equals(BleDefinedUUIDs.Characteristic.ROCHE_OTHER_UUID)) {
			mBleWrapper.mRoCharacteristicOthers = ch;
		}else {
			if(uuid.equals(BleDefinedUUIDs.Characteristic.H2BT_METER_CHARACTERISTIC)) {
				mBleWrapper.mH2BTCharacteristicMeter = ch;
				Log.e("---++---", "H2BT CHARACTERISTIC METER");
			}
		}
...
//		Log.i("H2BT","write cycle ... HAHA" + ch);
	}
	...
	public void newValueForCharacteristic(final BluetoothGattCharacteristic ch, final String strVal, final int intVal, final byte[] rawValue, final String timestamp) {
		...
		Log.e("H2BT","H2 DEBUG .....  newValueForCharacteristic");
	...
	public void setNotificationEnabledForService(final BluetoothGattCharacteristic ch) {
        /* Added by Jason */
        UUID uuid = ch.getUuid();
        Log.e("---++---", "H2BT NOTIFY (DETAIL) " + uuid + ", "+ ch);
        ...
        if (convertView == null) {
        	...
        	fields.readBtn.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					//mBleWrapper.mH2BTCharacteristicCable = ch;
					Log.e("---++---", "H2BT READING ...." + " " + mCharacteristic);
        	...
        	fields.writeBtn.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					Log.e("---++---", "H2BT WR BUTTON" + " " + mCharacteristic);
...
//					byte[] dataToWrite = parseHexStringToBytes(newValue);
					...
					mBleWrapper.writeDataToCharacteristic(mCharacteristic, guideOtherInit);
					//mBleWrapper.rocheFlowWriteTimeout();

                    //byte[] dataToWriteGetAll = { 0x01, 0x03, 0x01, 0x01, 0x00 };
                    //mBleWrapper.writeDataToCharacteristic(mCharacteristic, dataToWriteGetAll);
...
/*
					...
					mBleWrapper.writeDataToCharacteristic(mCharacteristic, dataToWriteExisting);
					// Added By Jason
					mBleWrapper.h2CmdSel = 0;
					mBleWrapper.mH2BTCharacteristic = mCharacteristic;
					mBleWrapper.addH2BTCharWriteTimeoutEOH();
					*/
				}
			});          	
        ...
        if((props & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
			...
			fields.notificationBtn.setEnabled((props & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0);
		}
