    	...
    	runOnUiThread(new Runnable() {
			...
			public void run() {
    			...
    			for(BluetoothGattService service : mBleWrapper.getCachedServices()) {
					...
					Log.e("---++---", "ROCHE SERVICE UUID " + uuid);

					if (uuid.equals(BleDefinedUUIDs.Service.BLE_BGM_SERVICE_UUID)) {
						mBleWrapper.mRocheServiceBgm = service;
						//mBleWrapper.bgmCharCtrlTimeout();
					}
					else if (uuid.equals(BleDefinedUUIDs.Service.BLE_DEV_INFO_UUID)) {
						mBleWrapper.mRocheServiceDevInfo = service;
						//mBleWrapper.bgmCharLoopTimeout();
					}
					else if (uuid.equals(BleDefinedUUIDs.Service.GLS_H2_SERVICE_UUID)) {
						Log.e("---++---", "H2 CABLE SERVICE .... " + uuid);
						mBleWrapper.mH2BTServiceCable = service;
						mBleWrapper.h2ServiceTimeout();
					}
					else if(uuid.equals(BleDefinedUUIDs.Service.ROCHE_OTHER_SERVICE_UUID)) {
						mBleWrapper.mRocheServiceOther = service;
				...
				//mBleWrapper.bgmCharCtrlTimeout();
				mBleWrapper.rocheOtherCharLoopTimeout();
